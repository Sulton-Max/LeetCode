...
namespace RegularExpMatching_10
{
    public class Solution
    {
        public bool IsMatch(string s, string p)
        {
            ...
            bool HasNextMatchingChar(char nextChar, int index)
            {
                do
                {
                    // Match for single char
                    if((p[index] != '*' && index + 1 == p.Length) || (p[index] != '*' && index + 1 < p.Length && p[index + 1] != '*'))
                    {
                        if (IsMatch(nextChar, p[index]))
                            return true;
                        else
                            return false;
                    }

                    // Match for cycling char
                    if(p[index] != '*' && index + 1 < p.Length && p[index + 1] == '*')
                    {
                        if (IsMatch(nextChar, p[index]))
                            return true;
                    }
                        ...
                        if (HasShouldMet(indexP, ref shouldMet, ref shouldMetNum))
                        {
                            ...
                            if (IsMatch(s[indexS], shouldMet))
                            {
                                ...
                                else // There no other should met chars, we should switch now
                                {
                                    ...
                                    if (indexS + 1 < s.Length && indexP + 1 < p.Length && HasNextMatchingChar(s[indexS + 1], indexP + 1) && s.Length != shouldMetNum && s[indexS + 1] != shouldMet)
                                    {
                                        // New Logic
